...
import { fmt, fmtR, fmtD, pc, uid, completeMonths } from "@/lib/helpers";
...
// ── Main tab ──────────────────────────────────────────────────────────────────
export default function ScenariosTab({ scenarios, groups, onScenariosChange, onGroupsChange, data, uiState={}, setUi=()=>{} }) {
  const activeId  = uiState.scenActiveId  || scenarios[0].id;
  const setActiveId = (id) => setUi({scenActiveId: id});
  const view      = uiState.scenView      || "edit";
  const setView   = (v) => setUi({scenView: v});
...
// ── Scenario projection ───────────────────────────────────────────────────────
// Day-by-day balance walkthrough for any selected scenario.
// All state persisted via uiState so nothing is lost on tab switch.
function ScenarioProjection({ scenarios, data, uiState, setUi }) {
  const complete = completeMonths(data.months);

  // ── Persisted state via uiState ─────────────────────────────────────────────
  const projScenId   = uiState.projScenId   ?? scenarios[0]?.id ?? "";
  const dayOverrides = uiState.projDayOverrides ?? {};   // {rowId: dayNum}
  const incomeDayOvr = uiState.projIncomeDay   ?? null;  // null = auto-detected
  const startBalOvr  = uiState.projStartBal    ?? null;  // null = auto from data

  const setField = (patch) => setUi(patch);

  const scenario  = scenarios.find(s=>s.id===projScenId) || scenarios[0];
  const incomeAmt = resolveIncome(scenario?.income, data);

  // ── Auto-detect typical income day from last 3 months ──────────────────────
  const incomeDays = complete.slice(-3).flatMap(m=>
    (m.transactions||[]).filter(t=>t.isIncome||t.amount>0)
      .map(t=>parseInt(t.date?.split("-")[2]||"0")).filter(Boolean)
  );
  const autoIncomeDay = incomeDays.length
    ? Math.round(incomeDays.reduce((a,b)=>a+b,0)/incomeDays.length) : 1;
  const incomeDay = incomeDayOvr ?? autoIncomeDay;

  // ── Auto-detect starting balance = start of most recent complete month ──────
  // This is the balance BEFORE any transactions that month — raw start of month.
  ...
  // Draft state for editable fields (local only, not persisted until blur)
  const [startDraft, setStartDraft] = useState(fmtR(startBal));
  const [incomeDraft,setIncomeDraft]= useState(String(incomeDay));

  // Sync drafts when persisted values change (e.g. on load)
  useState(() => { setStartDraft(fmtR(startBal)); });
  useState(() => { setIncomeDraft(String(incomeDay)); });

  // ── Build category → typical day map ────────────────────────────────────────
  const catDayAvg = {};
  ...
  (scenario?.rows||[]).filter(r=>r.enabled).forEach(row => {
    ...
    if (row.type === "fixed" || row.type === "percent") {
      ...
      const lastTx = (lastComplete?.transactions||[])
        .filter(t=>(row.liveCategories||[]).includes(t.category)&&t.amount<0);
    ...
    events.push({ day, label:row.name, amount:-amt, type:"expense",
      rowId:row.id, rowType:row.type, liveCategories:row.liveCategories });
  });
  ...
  // All rows that can have a day override
  const overridableRows = (scenario?.rows||[]).filter(r=>r.enabled);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:20}}>
        ...
        <div style={{display:"flex",gap:16,flexWrap:"wrap",alignItems:"flex-end"}}>
          ...
          <div>
            <div style={{color:C.textDim,fontSize:10,marginBottom:5}}>
              ...
              <span style={{color:C.muted,fontWeight:400,marginLeft:6}}>
                (auto: start of {lastComplete?.month??""} · {fmt(autoStartBal)})
              </span>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <button onClick={()=>{ setField({projStartBal:null}); setStartDraft(fmtR(autoStartBal)); }}
                style={{background:"transparent",border:`1px solid ${C.border}`,borderRadius:5,padding:"6px 10px",color:C.textDim,fontSize:10,cursor:"pointer",fontFamily:FONT}}>
                reset
              </button>
            </div>
          </div>
          ...
          <div>
            <div style={{color:C.textDim,fontSize:10,marginBottom:5}}>
              INCOME DAY
              <span style={{color:C.muted,fontWeight:400,marginLeft:6}}>(auto-detected: {autoIncomeDay})</span>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
      ...
      {/* Day overrides for all rows */}
      {overridableRows.length>0&&(
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:20}}>
            ...
            {overridableRows.map(row=>{
              ...
              return (
                <div key={row.id} style={{display:"flex",alignItems:"center",gap:8,background:C.elevated,borderRadius:7,padding:"8px 12px",border:`1px solid ${C.border}`}}>
                  ...
                  {auto!==null&&!dayOverrides[row.id]&&(
                    <span style={{color:C.muted,fontSize:9}}>auto:{auto}</span>
                  )}
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:20}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:0}}>
          ...
          {days.length===0&&(
            <div style={{color:C.muted,fontSize:12,padding:"20px 0",textAlign:"center"}}>No enabled rows — add items to the scenario to see the projection.</div>
          )}

          {days.map(({day,events:evs,balBefore,balAfter},di)=>(
                ...
                return (
                  <div key={ei} style={{display:"flex",alignItems:"center",gap:12,padding:"9px 0",background:di%2===0&&ei===0?`${C.elevated}55`:"transparent"}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      {ev.liveCategories?.length>0&&(
                        <div style={{color:C.muted,fontSize:9,marginTop:1}}>{ev.liveCategories.join(", ")}
                    ...
                    {isLastInDay?(
                      <div style={{minWidth:100,textAlign:"right"}}>
                        ...
                        <div style={{height:2,background:C.border,borderRadius:1,marginTop:4,width:100}}>
                          <div style={{height:"100%",borderRadius:1,background:runBal<0?C.red:runBal>=startBal?C.teal:C.amber,
                            width:`${Math.max(2,Math.min(100,((runBal-minBal)/balRange)*100))}%`,transition:"width 0.3s"}}/>
                        </div>
                      </div>
          ...
          {/* End row */}
          <div style={{display:"flex",alignItems:"center",gap:12,padding:"14px 0",marginTop:4,borderTop:`1px solid ${C.amberMid}`}}>
