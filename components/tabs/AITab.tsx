...
const saveHistory = (s: HistoryEntry[]) => {
  try { localStorage.setItem(AI_KEY, JSON.stringify(s.slice(-10))); } catch {}
};
interface HistoryEntry { date: string; months: string; summary: string; full?: string; }
...
export default function AITab({ data, markers, uiState = {}, setUi = () => {} }: AITabProps) {
  ...
  const setAiRange = (v: number)          => setUi({ aiRange: v });
  const setSelCats = (v: string[] | null) => setUi({ aiCats: v });
  const setMode    = (v: string)          => setUi({ aiMode: v });
  ...
  useEffect(() => {
    sGet(SK.ol).then(raw => {
      if (!raw) return;
      const cfg = raw as OllamaConfig;
      if (cfg.url)     setOUrl(cfg.url);
      if (cfg.model)   setModel(cfg.model);
      if (cfg.aiRange != null) setAiRange(cfg.aiRange);
      if (cfg.cats !== undefined) setSelCats(cfg.cats ?? null);
    });
  }, []);
  ...
  const doConnect = async () => {
    ...
    try {
      ...
      if (ms.length) {
        ...
        const best = pref.reduce<string | null>((f, p) => f ?? (ms.find(m => m.startsWith(p.split(":")[0])) ?? null), null) ?? ms[0];
        setModel(best);
  ...
  const buildContext = () => {
    ...
    const monthlyBlock = slice.map((m: import("@/types").Month) =>
      `${m.month}: in ${fmt(m.income)}, out ${fmt(m.expenses)}, net ${fmt(m.net)}, end ${fmt(m.endBalance)}\n` +
      activeCats.map(c => `  ${c}: ${fmt(catSpend(m, c))}`).join("\n")
    ).join("\n\n");
    const payeeMap: Record<string, { n: number; tot: number }> = {};
    allTx.forEach(t => {
      if (t.payee && t.amount < 0) {
        if (!payeeMap[t.payee]) payeeMap[t.payee] = { n: 0, tot: 0 };
        payeeMap[t.payee].n++;
        payeeMap[t.payee].tot += Math.abs(t.amount);
      }
    });
    const payeeBlock = Object.entries(payeeMap)
      .sort((a, b) => b[1].tot - a[1].tot).slice(0, 20)
      .map(([p, v]) => `${p}: ${fmt(v.tot)} (${v.n}x)`).join("\n");
  ...
  const streamOllama = async (
    ...
    onDone: (full: string, err: string | null) => void
  ) => {
    ...
    try {
      ...
      if (!resp.body) { onDone("", "No response body"); return; }
      ...
      while (!abortRef.current) {
        ...
        dec.decode(value).split("\n").filter(Boolean).forEach(line => {
          try {
            ...
            if (j.response) { full += j.response; onToken(full); }
          } catch {}
        });
      }
    } catch (e) { onDone(full, e instanceof Error ? e.message : String(e)); return; }
    onDone(full, null);
  };
    ...
    await streamOllama(prompt, setInsight, (full, err) => {
      ...
      if (full) {
        const entry: HistoryEntry = {
          ...
          months: `${slice[0]?.month}–${slice[slice.length - 1]?.month}`,
  ...
  const doChat = async () => {
    ...
    const { monthlyBlock, payeeBlock, txBlock } = buildContext();
    const context = `Financial data (GBP):\n${monthlyBlock}\n\nTop payees:\n${payeeBlock}\n\nTransactions:\n${txBlock}`;
    const history_block = messages.slice(-6).map(m => `${m.role === "user" ? "User" : "Assistant"}: ${m.content}`).join("\n");
    const prompt = `${context}\n\n${history_block}\nUser: ${userMsg.content}\nAssistant:`;
    const assistantMsg: Message = { role: "assistant", content: "" };
    setMessages(prev => [...prev, assistantMsg]);
    await streamOllama(prompt, full => {
      setMessages(prev => prev.map((m, i) => i === prev.length - 1 ? { ...m, content: full } : m));
    }, () => setChatLoading(false));
  };

  const { slice, txCount } = buildContext();

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {/* Ollama connection */}
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 20 }}>
        <div style={{ color: C.amber, fontSize: 10, letterSpacing: 2, marginBottom: 12 }}>OLLAMA CONNECTION</div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          <input value={oUrl} onChange={e => setOUrl(e.target.value)}
            style={{ flex: 2, minWidth: 200, background: C.bg, border: `1px solid ${C.border}`, borderRadius: 6, padding: "9px 12px", color: C.text, fontSize: 12, fontFamily: FONT, outline: "none" }} />
          <select value={model} onChange={e => setModel(e.target.value)}
            style={{ flex: 1, minWidth: 140, background: C.bg, border: `1px solid ${C.border}`, borderRadius: 6, padding: "9px 10px", color: C.text, fontSize: 12, fontFamily: FONT }}>
            {models.length ? models.map(m => <option key={m} value={m}>{m}</option>) : <option value={model}>{model}</option>}
          </select>
          <button onClick={doConnect}
            style={{ background: connected ? `${C.teal}22` : C.amber, color: connected ? C.teal : "#060e1a", border: `1px solid ${connected ? C.teal : C.amber}`, borderRadius: 6, padding: "9px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: FONT }}>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 22 }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {([{ v: "analyse", l: "📊 Analyse" }, { v: "chat", l: "💬 Ask a question" }] as const).map(({ v, l }) => (
            <button key={v} onClick={() => setMode(v)} style={{
              flex: 1, fontFamily: FONT, fontSize: 11, padding: "8px 0", borderRadius: 7, cursor: "pointer",
              border: `1px solid ${mode === v ? C.amber : C.border}`, background: mode === v ? `${C.amber}22` : "transparent",
      ...
      {mode === "analyse" && (
        <div style={{ background: C.surface, border: `1px solid ${C.amberLow}`, borderRadius: 10, padding: 22 }}>
          ...
          {loading && !insight && <div style={{ color: C.textDim, fontSize: 12, textAlign: "center", padding: "28px 0" }}><span style={{ color: C.amber }}>◈</span> {model} analysing…</div>}
          {insight && <div style={{ color: C.text, fontSize: 13, lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{insight}{loading && <span style={{ color: C.amber }}> ▋</span>}</div>}
          {!insight && !loading && <div style={{ color: C.textDim, fontSize: 12 }}>Runs a full analysis of the selected months. Good/bad markers and previous analyses are included automatically.</div>}
          {history.length > 0 && (
      ...
      {mode === "chat" && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, display: "flex", flexDirection: "column", minHeight: 400 }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px 18px", display: "flex", flexDirection: "column", gap: 12, minHeight: 280, maxHeight: 500 }}>
            {messages.length === 0 && <div style={{ color: C.muted, fontSize: 12, textAlign: "center", padding: "40px 0" }}>{connected ? "Type a question below…" : "Connect to Ollama above first."}</div>}
            {messages.map((msg, i) => (
              <div key={i} style={{ alignSelf: msg.role === "user" ? "flex-end" : "flex-start", maxWidth: "85%", background: msg.role === "user" ? `${C.amber}22` : C.elevated, border: `1px solid ${msg.role === "user" ? C.amber : C.border}`, borderRadius: 10, padding: "10px 14px" }}>
          ...
          <div style={{ padding: "12px 16px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 10 }}>
            <input value={input} onChange={e => setInput(e.target.value)}
              ...
              style={{ flex: 1, background: C.bg, border: `1px solid ${C.border}`, borderRadius: 6, padding: "9px 12px", color: C.text, fontSize: 12, fontFamily: FONT, outline: "none" }} />
            <button onClick={doChat} disabled={!connected || chatLoading || !input.trim()}
              style={{ background: connected && input.trim() ? C.amber : "transparent", color: connected && input.trim() ? "#060e1a" : C.muted, border: `1px solid ${connected && input.trim() ? C.amber : C.border}`, borderRadius: 6, padding: "9px 16px", fontSize: 12, fontWeight: 700, cursor: connected && input.trim() ? "pointer" : "default", fontFamily: FONT }}>
              {chatLoading ? "…" : "SEND"}
            </button>
